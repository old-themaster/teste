...
namespace Game.Server.Rooms
{
    public class RoomMgr
    {
        ...
        public static BaseRoom[] Rooms => RoomMgr.baseRoom_0;

        public static BaseWaitingRoom WaitingRoom => RoomMgr.baseWaitingRoom_0;

        public static BaseChristmasRoom ChristmasRoom
        {
            get
            {
                return m_christmasRoom;
            }
        }
        ...
        public static bool Setup(int maxRoom)
        {
            ...
            RoomMgr.m_christmasRoom = new BaseChristmasRoom();
        ...
        public static void CreateRoom(
          ...
          string password,
          eRoomType roomType,
          byte timeType) => RoomMgr.AddAction((IAction)new CreateRoomAction(player, name, password, roomType, timeType));
        ...
        public static void CreateSingleRoom(
          ...
          string password,
          eRoomType roomType,
          int mapId) => RoomMgr.AddAction((IAction)new SingleRoomCreateAction(player, name, password, roomType, mapId));
        ...
        public static void EnterRoom(
          ...
          string pwd,
          int type,
          bool isInvite) => RoomMgr.AddAction((IAction)new EnterRoomAction(player, roomId, pwd, type, isInvite));
        public static void CreateCatchBeastRoom(GamePlayer player)
        {
            RoomMgr.AddAction(new CreateCatchBeastAction(player));
        }
        public static void EnterRoom(GamePlayer player) => RoomMgr.EnterRoom(player, -1, (string)null, 1, true);
        ...
        public static void ExitRoom(BaseRoom room, GamePlayer player) => RoomMgr.AddAction((IAction)new ExitRoomAction(room, player, false));

        public static void StartGame(BaseRoom room) => RoomMgr.AddAction((IAction)new StartGameAction(room));

        public static void UpdatePlayerState(GamePlayer player, byte state) => RoomMgr.AddAction((IAction)new UpdatePlayerStateAction(player, player.CurrentRoom, state));

        public static void UpdateRoomPos(BaseRoom room, int pos, bool isOpened) => RoomMgr.AddAction((IAction)new UpdateRoomPosAction(room, pos, isOpened));

        public static void KickPlayer(BaseRoom baseRoom, byte index) => RoomMgr.AddAction((IAction)new KickPlayerAction(baseRoom, (int)index));

        public static void EnterWaitingRoom(GamePlayer player) => RoomMgr.AddAction((IAction)new EnterWaitingRoomAction(player));

        public static void ExitWaitingRoom(GamePlayer player) => RoomMgr.AddAction((IAction)new ExitWaitRoomAction(player));

        public static void CancelPickup(BattleServer server, BaseRoom room) => RoomMgr.AddAction((IAction)new CancelPickupAction(server, room));
        ...
        string roomPass,
        bool isCross,
        int currentFloor) => RoomMgr.AddAction((IAction)new RoomSetupChangeAction(room, roomType, timeMode, hardLevel, levelLimits, mapId, roomName, roomPass, isCross, currentFloor));
       ...
       /* public static void UpdateRoomGameType(BaseRoom room, eRoomType roomType, byte timeMode, eHardLevel hardLevel, int levelLimits, int mapId, string password, string roomname, bool isCrosszone, bool isOpenBoss, string Pic, int currentFloor)
        {
            RoomMgr.AddAction(new RoomSetupChangeAction(room, roomType, timeMode, hardLevel, levelLimits, mapId, password, roomname, isCrosszone, isOpenBoss, Pic, currentFloor));
        }*/

        internal static void smethod_2(GamePlayer gamePlayer_0) => RoomMgr.AddAction((IAction)new SwitchTeamAction(gamePlayer_0));

        public static void StartGameMission(BaseRoom room) => RoomMgr.AddAction((IAction)new StartGameMissionAction(room));

        public static void CreateConsortiaBossRoom(GamePlayer player, eRoomType roomType, int bossLevel)
        {
            RoomMgr.AddAction(new CreateConsortiaBossAction(player, roomType, bossLevel));
        }
        ...
        {
            lock (RoomMgr.baseRoom_0)
                return id < RoomMgr.baseRoom_0.Length ? RoomMgr.baseRoom_0[id] : (BaseRoom)null;
        }
        ...
        public static List<BaseRoom> GetAllPveRooms()
        {
            ...
            BaseRoom[] rooms;
            Monitor.Enter(rooms = RoomMgr.baseRoom_0);
            try
            {
                BaseRoom[] rooms2 = RoomMgr.baseRoom_0;
        ...
        public static void StartProxyGame(BaseRoom room, ProxyGame game) => RoomMgr.AddAction((IAction)new StartProxyGameAction(room, game));

        public static void StopProxyGame(BaseRoom room) => RoomMgr.AddAction((IAction)new StopProxyGameAction(room));
    }
}
