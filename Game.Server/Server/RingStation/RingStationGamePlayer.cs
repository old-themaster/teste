...
namespace Game.Server.RingStation
{
  public class RingStationGamePlayer
  {
    ...
    public static double ComputeVx(double dx, float m, float af, float f, float t) => (dx - (double) f / (double) m * (double) t * (double) t / 2.0) / (double) t + (double) af / (double) m * dx * 0.8;

    public static double ComputeVy(double dx, float m, float af, float f, float t) => (dx - (double) f / (double) m * (double) t * (double) t / 2.0) / (double) t + (double) af / (double) m * dx * 1.3;
    ...
    public void NextTurn(GSPacketIn pkg) => this.SendSelfTurn(pkg.Parameter1 == this.GamePlayerId, true);

    public void Pause(int time) => this.m_passTick = Math.Max(this.m_passTick, TickHelper.GetTickCount() + (long) time);
    ...
    public void Resume() => this.m_passTick = 0L;
    ...
    private void SendSelfTurn(bool fire) => this.SendSelfTurn(fire, false);
    ...
    internal void SendTCP(GSPacketIn pkg) => this.CurRoom.SendTCP(pkg);

    public void SendUseProp(int templateId) => this.SendUseProp(templateId, 0, 0, 0, 0);
    ...
    {
      get => this._canUserProp;
      set => this._canUserProp = value;
    }
