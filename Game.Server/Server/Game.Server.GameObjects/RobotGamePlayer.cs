...
namespace Game.Server.GameObjects
{
    internal class RobotGamePlayer : GamePlayer
    {
        ...
        public class NullOut : IPacketLib
        {
			...
			public GSPacketIn SendSingleRoomCreate(BaseRoom room, int zoneId) => null;
			...
			public GSPacketIn SendUpdateConsotiaBuffer(GamePlayer player, Dictionary<string, BufferInfo> bufflist) => null;
			...
			public GSPacketIn SendUserEquip(PlayerInfo player, List<ItemInfo> items) => null;
			...
			public GSPacketIn SendUpdatePlayerProperty(PlayerInfo info, PlayerProperty PlayerProp) => null;

			public GSPacketIn SendOpenVIP(GamePlayer Player) => null;

			public GSPacketIn SendUserRanks(List<UserRankInfo> rankList) => null;
			...
			public void SendLeagueNotice(int id, int restCount, int maxCount, byte type) => Nothing();
