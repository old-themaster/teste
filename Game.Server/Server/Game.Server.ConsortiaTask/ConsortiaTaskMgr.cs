...
namespace Game.Server.ConsortiaTask
{
    public class ConsortiaTaskMgr
	{
		...
		public static void ScanConsortiaTask()
		{
			lock (object_0)
			{
				foreach (BaseConsortiaTask baseConsortiaTask in GetAllConsortiaTaskData())
				{
					if (baseConsortiaTask.Info.CanRemove || baseConsortiaTask.Finish())
					{
						baseConsortiaTask.ClearAllUserData();
						RemoveConsortiaTask(baseConsortiaTask.Info.ConsortiaID);
					}
				}
			}
		}
		...
		public static bool AddConsortiaTask(int consortiaId, int level)
		{
			...
			{
				BaseConsortiaTask baseConsortiaTask2 = new BaseConsortiaTask(new ConsortiaTaskDataInfo(consortiaId, int_1[level - 1], int_0[level - 1], int_2[level - 1], GetRandomConsortiaBuff(level).id, GameProperties.MissionMinute), GetRandomTaskCondition(4, 3));
				lock (dictionary_1)
				{
