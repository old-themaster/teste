...
namespace Game.Server.Packets.Client
{
	...
	public class UserChangeItemPlaceHandler : IPacketHandler
	{
		...
		public int HandlePacket(GameClient client, GSPacketIn packet)
		{
			...
			ItemTemplateInfo temp = ItemMgr.FindItemTemplate(itemAt.TemplateID);
			if (inventory != null && itemAt != null)
			{
				if (num3 >= 0 && num3 <= itemAt.Count)
				{
					...
					try
					{
						...
						{
							log.Error((object)("userid: " + client.Player.PlayerCharacter.ID + " - uname: " + client.Player.PlayerCharacter.UserName + " hack place " + num2));
							GamePlayer[] allPlayers = WorldMgr.GetAllPlayers();
							foreach (GamePlayer gamePlayer in allPlayers)
							{
								if (ComandosMgr.checkStaff(gamePlayer.PlayerCharacter.ID))
								{
									GSPacketIn pkg = WorldMgr.SendStaffHelp("O jogador: [" + client.Player.PlayerCharacter.NickName + "] está tentando usar HACK UseChangeItemPlaceHandler");
						...
						{
							client.Player.SendMessage("Mochila cheia, por favor, limpe e tente novamente.");
							return 0;
						}
						...
						if (num2 == -1)
						{
							...
							else if (eBageType == eBageType.Consortia || eBageType == eBageType.BankBag)
							{
								num2 = inventory2.FindFirstEmptySlot();
								if (num2 != -1)
								{
							...
							else if (eBageType2 == eBageType.Consortia || eBageType2 == eBageType.BankBag)
							{
								num2 = inventory2.FindFirstEmptySlot();
								if (num2 != -1)
								{
								...
								case eBageType.BankBag:
									if (eBageType2 == eBageType.EquipBag && num2 < 31)
									{
										...
										foreach (GamePlayer gamePlayer2 in allPlayers2)
										{
											...
											{
												GSPacketIn pkg2 = WorldMgr.SendStaffHelp("O jogador: [" + client.Player.PlayerCharacter.NickName + "] está tentando usar HACK UseChangeItemPlaceHandler");
