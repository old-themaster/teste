            ...
            else if (itemAt.Count >= count && count > 0)
            {
                ...
                using (PlayerBussiness playerBussiness = new PlayerBussiness())
                {
                    if (playerBussiness.AddAuction(info))
                    {
                        ...
                        GameServer.Instance.LoginServer.SendPacket(WorldMgr.SendSysNotice(eMessageType.ChatNormal, LanguageMgr.GetTranslation("GetAuctionHandlerSystem.Msg", (object)client.Player.PlayerCharacter.NickName, (object)info.TemplateID, (object)info.Price), info.ItemID, info.TemplateID, (string)null));
