...
namespace Game.Server.Packets.Client
{
    ...
    public class SceneChatHandler : IPacketHandler
    {
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
                ...
                if (mensagem[0].Equals("@@"))
                {
                    if (verificarAdm(client.Player.PlayerCharacter.NickName))
                    {
                        ...
                        if (mensagem[1].Equals("all"))
                        {
                            ...
                            foreach (GamePlayer gamePlayer in WorldMgr.GetAllPlayers())
                            {
                                gamePlayer.SendMessage(string.Format("[Sistema]: {0}", (object)message));
                            }
                        ...
                        if (mensagem[1].Equals("privado"))
                        {
                            string nickname = mensagem[2];
                            string message = mensagem[3];
                        ...
                        if (mensagem[1].Equals("privado1"))
                        {
                            ...
                            if (gamePlayer != null)
                            {
                                ...
                                string formattedMessage = string.Format("{0} está enviando uma mensagem para você: {1}", client.Player.PlayerCharacter.NickName, message);

                                // Envia a mensagem para o jogador
                                gamePlayer.SendMessage(formattedMessage);
                            }
                            else
                            {
                                Console.WriteLine($"Jogador com nickname {nickname} não encontrado.");
            ...
            if (client.Player.PlayerCharacter.Grade < 13)
            {
                client.Player.SendMessage("Você precisa ter nível 13 ou superior para falar no chat.");
                return 1;
            }
