...
namespace GameServerScript.AI.NPC
{
    public class FiveNormalFirstBoss : ABrain
    {
        ...
        private int m_bossId = 5101;

        private int m_npcId = 5102;

        private int m_reduceStreng = 50;

        private int m_reduceBlood = 200;
        ...
        public override void OnStartAttacking()
        {
            ...
            {
                if (p.IsActive && p.IsLiving && p.X > 1199)
                    pneedkill.Add(p);
                else if (p.IsActive && p.IsLiving && p.X < 290 && p.Y < 555)
                    pneedkill.Add(p);
            }
        ...
        private void CallNewNpc()
        {
            ...
            SimpleNpc newNpc = ((PVEGame)Game).CreateNpc(m_npcId, m_friendNpc.X, m_friendNpc.Y, 0, 1, config);
