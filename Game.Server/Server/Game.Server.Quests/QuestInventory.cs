		...
		public bool Finish(BaseQuest baseQuest, int selectedItem)
		{
			...
			try
			{
				if (baseQuest.Finish(m_player))
				{
					...
					foreach (QuestAwardInfo info2 in questGoods)
					{
						...
						for (int i = 0; i < rewardItemCount; i += goods.MaxCount)
						{
							...
							if (item != null)
							{
								...
								if (goods.TemplateID == 11183)
								{
									m_player.LoadAscensions();
									m_player.OnPlayerAddItem("Ascension", count);
								}
							}
						}
					}
					...
					if (items.Count > 0)
					{
						m_player.SendItemsToMail(items, "Bag is full", "Item enviado do sistema", eMailType.ItemOverdue);
