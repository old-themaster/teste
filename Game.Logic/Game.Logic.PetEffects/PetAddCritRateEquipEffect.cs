...
// Type: Game.Logic.PetEffects.PetAddCritRateEquipEffect
...
namespace Game.Logic.PetEffects
{
  public class PetAddCritRateEquipEffect : BasePetEffect
  {
    ...
    public PetAddCritRateEquipEffect(
      int count,
      int probability,
      int type,
      int skillId,
      int delay,
      string elementID)
      : base(ePetEffectType.PetAddCritRateEquipEffect, elementID)
    {
    ...
    private void player_AfterKilledByLiving(Living living)
    {
      if (this.rand.Next(10000) >= this.m_probability || living.PetEffects.CurrentUseSkill != this.m_currentId)
        return;
      new PetAddCritRateEquip(this.m_count, this.m_currentId, this.Info.ID.ToString()).Start(living);
    }
