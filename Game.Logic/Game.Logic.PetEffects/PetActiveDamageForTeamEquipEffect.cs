  ...
  public class PetActiveDamageForTeamEquipEffect : BasePetEffect
  {
    ...
    private void player_AfterKilledByLiving(Living living)
    {
      if (this.rand.Next(10000) >= this.m_probability || living.PetEffects.CurrentUseSkill != this.m_currentId)
        return;
      new PetShootedAddDamageForTeamEquip(this.m_count, this.m_currentId, this.Info.ID.ToString()).Start(living);
    }

    public override bool Start(Living living)
    {
      PetActiveDamageForTeamEquipEffect ofType = living.PetEffectList.GetOfType(ePetEffectType.PetActiveDamageForTeamEquipEffect) as PetActiveDamageForTeamEquipEffect;
      if (ofType == null)
