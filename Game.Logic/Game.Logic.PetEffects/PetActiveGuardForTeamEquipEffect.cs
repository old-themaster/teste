...
// Type: Game.Logic.PetEffects.PetActiveGuardForTeamEquipEffect
...
namespace Game.Logic.PetEffects
{
  public class PetActiveGuardForTeamEquipEffect : BasePetEffect
  {
    ...
    public PetActiveGuardForTeamEquipEffect(
      int count,
      int probability,
      int type,
      int skillId,
      int delay,
      string elementID)
      : base(ePetEffectType.PetActiveGuardForTeamEquipEffect, elementID)
    {
      this.m_count = count;
    ...
    private void player_AfterKilledByLiving(Player living)
    {
      if (this.rand.Next(10000) >= this.m_probability || living.PetEffects.CurrentUseSkill != this.m_currentId)
        return;
      new PetShootedAddGuardForTeamEquip(this.m_count, this.m_currentId, this.Info.ID.ToString()).Start((Living) living);
    }
