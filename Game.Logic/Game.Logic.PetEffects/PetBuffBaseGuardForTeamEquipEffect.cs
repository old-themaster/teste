...
// Type: Game.Logic.PetEffects.PetBuffBaseGuardForTeamEquipEffect
...
namespace Game.Logic.PetEffects
{
  public class PetBuffBaseGuardForTeamEquipEffect : BasePetEffect
  {
    ...
    public PetBuffBaseGuardForTeamEquipEffect(
      int count,
      int probability,
      int type,
      int skillId,
      int delay,
      string elementID)
      : base(ePetEffectType.PetBuffBaseGuardForTeamEquipEffect, elementID)
    {
      this.m_count = count;
    ...
    private void ChangeProperty(Player player)
    {
      ...
      foreach (Living allTeamPlayer in player.Game.GetAllTeamPlayers((Living) player))
        allTeamPlayer.AddPetEffect((AbstractPetEffect) new PetBuffGuardEquip(this.m_count, this.m_currentId, this.Info.ID.ToString()), 0);
    }
