...
namespace Game.Logic
{
	public static class PveInfoMgr
	{
		...
		public static bool ReLoad()
		{
			...
			{
				ilog_0.Error("PveInfoMgr", exception);
			}
			return false;
		}
		...
		public static Dictionary<int, PveInfo> LoadFromDatabase()
		{
			...
			using (PveBussiness pveBussiness = new PveBussiness())
			{
				...
				foreach (PveInfo pveInfo in allPveInfos)
				{
		...
		public static PveInfo GetPveInfoByType(eRoomType roomType, int levelLimits)
		{
			...
			case eRoomType.Exploration:
				foreach (PveInfo value in dictionary_0.Values)
				{
					if (value.Type == (int)roomType && value.LevelLimits == levelLimits)
					{
						return value;
					}
				}
		    ...
		    case eRoomType.SpecialActivityDungeon:
					foreach (PveInfo value2 in dictionary_0.Values)
				{
					if (value2.Type == (int)roomType)
					{
						return value2;
					}
				}
