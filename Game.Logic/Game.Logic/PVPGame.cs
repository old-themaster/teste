...
namespace Game.Logic
{
    public class PVPGame : BaseGame
    {
        ...
        public void GameOver()
        {
            ...
            foreach (Player player in allFightPlayers)
            {
                ...
                if (this.RoomType == eRoomType.Match)
                {
                    ...
                    int num70 = 1;
                    int num15 = 1;
                    this.Random.Next(PVPGame.MONEY_MIN_RATE_LOSE, PVPGame.MONEY_MAX_RATE_LOSE);
                    this.Random.Next(PVPGame.EXP_MIN_RATE_LOSE, PVPGame.EXP_MAX_RATE_LOSE);
                    this.Random.Next(PVPGame.MONEY_MIN_RATE_LOSE, PVPGame.MONEY_MAX_RATE_LOSE);
                    this.Random.Next(PVPGame.EXP_MIN_RATE_LOSE, PVPGame.EXP_MAX_RATE_LOSE);
                    ...
                    Convert.ToDateTime("12:30:00");
                    DateTime dateTime1 = Convert.ToDateTime("19:30:00");
                    DateTime dateTime2 = Convert.ToDateTime("21:30:00");
                    DateTime datetimeeliga1 = Convert.ToDateTime("19:30:00");
                    DateTime datetimeeliga2 = Convert.ToDateTime("21:30:00");
                    if (player.TotalHurt > 0)
                    {
                        if (DateTime.Now >= dateTime1 && DateTime.Now <= dateTime2)
                        {
                            num10 = 2;
                            player.PlayerDetail.SendMessage("Aproveite Você esta jogando em horário de dobro de cupons.");
                        }
                        int num13;
                        if (val2)

                        {
                            int leagueMoneyWin = PVPGame.LeagueMoney_Win;
                             ...
                             num13 = random1.Next(200, 200) * num10 + num11;
                            num9 += 5 * num10 + num12;
                              msg = LanguageMgr.GetTranslation("PVPGame.SendGameOVer.Msg1", (object)num13);

                        }
                        else
                        {
                            int leagueMoneyLose2 = PVPGame.LeagueMoney_Lose;
                             num13 = random1.Next(100, 100) * num10 + num11;
                            num9 += 5 * num10 + num12;
                             msg = LanguageMgr.GetTranslation("PVPGame.SendGameOVer.Msg1", (object)num13);
                        }
                        ...
                        player.PlayerDetail.SendMessage("Você não sofre nenhum dano ao inimigo e não recebe experiência ou cupons!!");
                    else
                        player.PlayerDetail.SendMessage("Você não causou  nenhum dano ao inimigo e não recebe experiência ou cupons.!");
                    num9 += 5;
                    if (msg != "" && msg != null)
                ...
                if (player.PlayerDetail.PlayerCharacter.VIPLevel > (byte)0)
                {
                    num9 += 10;
                    ++val1;
                }
        ...
        public void NextTurn()
        {
            ...
            if (m_currentLiving is Player && (base.RoomType == eRoomType.EntertainmentRoom || base.RoomType == eRoomType.EntertainmentRoomPK))
            {
                (m_currentLiving as Player).PlayerDetail.CreateRandomEnterModeItem();
            }
        ...
        {
            if (base.RoomType != eRoomType.EntertainmentRoom && base.RoomType != eRoomType.EntertainmentRoomPK)
            {
                return false;
            }
            return true;
        ...
        public void GameOverEntertainmentMode()
        {
            ...
            {
                player2.CanTakeOut = ((player2.Team == 1) ? num3 : num2);
                bool val = false;
                val = ((base.RoomType != eRoomType.FightFootballTime) ? (player2.Team == team) : ((player2.Team == 1 && blueScore > redScore) || (player2.Team == 2 && blueScore < redScore)));
                int num4 = 0;
                switch (typeEnterMode)
                {
                    case 1:
                        num4 = ((!val) ? 4 : 10);
                        break;
                ...
                if (num4 > 0)
                {
                    player2.PlayerDetail.AddEnterModePoint(num4);
                    player2.PlayerDetail.SendMessage("VocÇe Ganhou " + num4 + " Pontos");
                }
                if (base.RoomType == eRoomType.EntertainmentRoomPK && val)
                {
                    player2.PlayerDetail.AddMoney(180);
                    player2.PlayerDetail.SendMessage("Você Ganhou 180 Cupons");
                }
                player2.PlayerDetail.ClearFightBag();
                pkg.WriteInt(player2.Id);
                pkg.WriteBoolean(val);
                pkg.WriteInt(player2.Grade);
                pkg.WriteInt(player2.PlayerDetail.PlayerCharacter.GP);
                pkg.WriteInt(0);
                pkg.WriteInt(0);
                pkg.WriteInt(0);
                pkg.WriteInt(0);
                ...
                pkg.WriteInt(0);
                pkg.WriteInt(0);
                pkg.WriteInt(player2.GainOffer);
                pkg.WriteInt(0);
                pkg.WriteInt(player2.CanTakeOut);
            }
            pkg.WriteInt(0);
            pkg.WriteInt(0);
            pkg.WriteInt(0);
            ...
            {
                player3.PlayerDetail.OnGameOver(this, player3.Team == team, player3.GainGP);
            }
            string str = "";
            OnGameOverLog(base.RoomId, base.RoomType, base.GameType, 0, beginTime, DateTime.Now, BeginPlayerCount, base.Map.Info.ID, teamAStr, teamBStr, str, team, BossWarField);
            OnGameOverred();
            lock (m_players)
            {
                m_players.Clear();
        ...
        /* public void StartGame()
          {
              ...
              pkg.WriteString(string.Format("Map : {0}, ID : {1}", this.m_map.Info.Name, this.m_map.Info.ID));
        ...
        public void StartGame()
        {
            ...
            //pkg1.WriteString(string.Format("Os pets estão desabilitados nos combates"));
