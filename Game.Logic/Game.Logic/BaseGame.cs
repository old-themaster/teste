...
namespace Game.Logic
{
    public class BaseGame : AbstractGame
    {
        ...
        20
            ...
            this.Cards = this.RoomType != eRoomType.Dungeon || base.RoomType == eRoomType.SpecialActivityDungeon ? new int[9] : new int[21];
        ...
        public SimpleBoss FindBossWithID(int id)
        {
            ...
            foreach (Living list0 in this.list_0)
            {
        ...
        public void LivingChangeAngle(Living living_0, int int_4, int int_5, string string_0)
        {
            GSPacketIn gSPacketIn = new GSPacketIn(91, living_0.Id)
            {
                Parameter1 = living_0.Id
        ...
        public List<SimpleNpc> GetNPCLivingWithProperties1(int properties)
        {
            List<SimpleNpc> simpleNpcs = new List<SimpleNpc>();
            foreach (Living mLiving in this.m_livings)
            {
                if (!(mLiving is SimpleNpc) || !mLiving.IsLiving || (mLiving as SimpleNpc).Properties4 != properties)
                {
                    continue;
                }
        ...
        internal void method_5(Player player_0, int int_4, bool bool_1)
        {
            GSPacketIn gSPacketIn = new GSPacketIn(91)
            {
                Parameter1 = player_0.Id
            };
            gSPacketIn.WriteByte(153);
            gSPacketIn.WriteInt(int_4);
            gSPacketIn.WriteBoolean(bool_1);
            if (player_0.Id == int_4)
            {
                this.SendToAll(gSPacketIn);
                return;
            }
            player_0.PlayerDetail.SendTCP(gSPacketIn);
        }
        internal void method_69(Living living_0, int int_4, bool bool_1)
        {
            GSPacketIn gSPacketIn = new GSPacketIn(91)
            {
                Parameter1 = living_0.Id
        ...
        internal void method_39(Living living_0)
        {
            GSPacketIn gSPacketIn = new GSPacketIn(91, living_0.Id)
            {
                Parameter1 = living_0.Id
            ...
            gSPacketIn.WriteBoolean(living_0.IsHide);
            this.SendToAll(gSPacketIn);
        }
        internal void method_37(Living living_0)
        {
            GSPacketIn gSPacketIn = new GSPacketIn(91, living_0.Id)
            {
                Parameter1 = living_0.Id
            ...
            gSPacketIn.WriteBoolean(living_0.IsFrost);
            this.SendToAll(gSPacketIn);
        }
        internal void method_40(Living living_0, string string_0, string string_1)
        {
            GSPacketIn gSPacketIn = new GSPacketIn(91, living_0.Id)
            {
                Parameter1 = living_0.Id
        ...
        internal void method_6(Player player_0)
        {
            GSPacketIn gSPacketIn = new GSPacketIn(91);
        ...
        internal void SendStartLoading(int maxTime)
        {
            ...
            if (this.isTrainer() || this.RoomType == eRoomType.FightFootballTime)
            {
                pkg.WriteInt(0);
            }
            else
        ...
        public bool GameIs(int Count)
        {
            if (Players.Values.Count((Player s) => s.Team == 1) < Count || Players.Values.Count((Player s) => s.Team != 1) < Count)
            {
                return false;
            }
            if (Players.Values.Count((Player s) => s.Team == 1) == Players.Values.Count((Player s) => s.Team != 1) && Players.Values.Count((Player s) => s.Team != 1) == Count)
            {
                return true;
            }
            return false;
        ...
        {
            foreach (WindInfo item in WindMgr.GetWind())
            {
        ...
        internal void SendSelectObject(int playerId)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                gSPacketIn.WriteInt(Players[i].PlayerDetail.ZoneId);
