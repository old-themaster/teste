	...
	public class WindMgr
	{
		...
		public static byte[] CreateVane(string wind)
		{
			...
			try
			{
				...
				int num2 = rand.Next(WindMgr.font.Length);
				Font font = new Font(WindMgr.font[num2], 17f, FontStyle.Italic);
				Point point = new Point(8, 12);
				if (isSmall(wind))
				{
					if (wind == fontWind[0])
					{
						font = new Font(WindMgr.font[num2], 10f, FontStyle.Regular);
					}
					point = new Point(4, 16);
				}
				float num3 = ThreadSafeRandom.NextStatic(-num, num);
				graphics.TranslateTransform(point.X, point.Y);
				graphics.RotateTransform(num3);
				graphics.DrawString(wind.ToString(), font, brush, 1f, 1f, format);
				graphics.RotateTransform(0f - num3);
