...
namespace Center.Server
{
    public class CenterServer : BaseServer
    {
        ...
        public bool InitGlobalTimers()
        {
            ...
            if (this.m_sytemNotice == null)
                this.m_sytemNotice = new System.Threading.Timer(new TimerCallback(this.SystemNoticeTimerProc), (object)null, num2, num2);
            else
                this.m_sytemNotice.Change(num2, num2);
            int num3 = this._config.LoginLapseInterval * 60 * 1000;
            if (this.m_loginLapseTimer == null)
                this.m_loginLapseTimer = new System.Threading.Timer(new TimerCallback(this.LoginLapseTimerProc), (object)null, num3, num3);
            else
                this.m_loginLapseTimer.Change(num3, num3);
            int num4 = this._config.SaveRecordInterval * 60 * 1000;
            if (this.m_saveRecordTimer == null)
                this.m_saveRecordTimer = new System.Threading.Timer(new TimerCallback(this.SaveRecordProc), (object)null, num4, num4);
            else
                this.m_saveRecordTimer.Change(num4, num4);
            int num5 = this._config.ScanAuctionInterval * 60 * 1000;
            if (this.m_scanAuction == null)
                this.m_scanAuction = new System.Threading.Timer(new TimerCallback(this.ScanAuctionProc), (object)null, num5, num5);
            else
                this.m_scanAuction.Change(num5, num5);
            int num6 = this._config.ScanMailInterval * 60 * 1000;
            if (this.m_scanMail == null)
                this.m_scanMail = new System.Threading.Timer(new TimerCallback(this.ScanMailProc), (object)null, num6, num6);
            else
                this.m_scanMail.Change(num6, num6);
            int num7 = this._config.ScanConsortiaInterval * 60 * 1000;
            if (this.m_scanConsortia == null)
                this.m_scanConsortia = new System.Threading.Timer(new TimerCallback(this.ScanConsortiaProc), (object)null, num7, num7);
            else
                this.m_scanConsortia.Change(num7, num7);
            int num8 = 60000;
            if (this.m_worldEvent == null)
                this.m_worldEvent = new System.Threading.Timer(new TimerCallback(this.ScanWorldEventProc), (object)null, num8, num8);
            else
                this.m_worldEvent.Change(num8, num8);
            int num9 = 60000;
            if (this.m_consortiaboss == null)
                this.m_consortiaboss = new System.Threading.Timer(new TimerCallback(this.ScanConsortiabossProc), (object)null, num9, num9);
            else
        ...
        public bool SendReload(eReloadType type) => this.SendReload(type.ToString());
        ...
        public void SendShutdown() => this.SendToALL(new GSPacketIn((short)15));
        ...
        public void SendToALL(GSPacketIn pkg) => this.SendToALL(pkg, (ServerClient)null);
        ...
        public void SendUpdateWorldEvent()
        {
            ...
            if ((hour >= 9 && hour < 10) || (hour >= 18 && hour < 19))
            {
                if (!WorldMgr.IsBattleGoundOpen)
                {
                    ...
                    this.SendSystemNotice("A arena está aberta. Apresse-se e junte-se a Gunners.");
                }
            }
            else
            {
                if (WorldMgr.IsBattleGoundOpen)
                {
                    ...
                    this.SendSystemNotice("A arena de hoje acabou.");
                }
            }
            if (hour >= 22 && hour < 23)
            {
                if (!WorldMgr.IsLeagueOpen)
                {
                    ...
                    this.SendSystemNotice("Começou o tempo da liga do torneio 2v2.");
                }
            }
            else
            {
                if (WorldMgr.IsLeagueOpen)
                {
                    ...
                    this.SendSystemNotice("Sistema de torneio fechado...");
                }
            }
            ...
            if (WorldMgr.IsFightFootballTime)
            {
                ...
                if (!this.AvailTime(WorldMgr.FightFootballTime, min))
                {
                    ...
                    this.SendSystemNotice("A batalha decisiva de hoje acabou.");
                }
            }
            else
            {
                if (hour.ToString() == array[0] && !WorldMgr.IsFightFootballTime)
                {
                    this.SendFightFootballTime(true);
                    WorldMgr.IsFightFootballTime = true;
                    WorldMgr.FightFootballTime = DateTime.Now;
                    this.SendSystemNotice("A batalha decisiva foi aberta. Apresse-se e junte-se a Gunners.");
                }
            }



            if (GuildBalancemode.CheckStartTime())
            {
                this.SendSystemNotice("O GVG Equilibrado está ativo");
            }


            if (!WorldMgr.worldOpen)
            {
                if (hour == 13 & boss == true)
                {
                    this.SendPrivateInfo();
                    WorldMgr.SetupWorldBoss(0);
                    this.SendSystemNotice(string.Format("Chefe do mundo {0} está aberto, inscreva-se agora!", WorldMgr.name[0]));
                    boss = true;
                }
                else
                {
                    if (hour == 21 && boss == true)
                    {
                        this.SendPrivateInfo();
                        WorldMgr.SetupWorldBoss(3);
                        this.SendSystemNotice(string.Format("Chefe do mundo {0} está aberto, inscreva-se agora", WorldMgr.name[3]));
                        boss = true;
                    }
                    else
                    {
                        ...
                        {
                            this.SendPrivateInfo();
                            WorldMgr.SetupWorldBoss(2);
                            this.SendSystemNotice(string.Format("Chefe do mundo {0} está aberto, inscreva-se agora", WorldMgr.name[2]));
                            boss = true;
                        }

                        else
                        {
                            this.SendRoomClose(2);
            ...
            if (WorldMgr.worldOpen)
            {
                ...
                if (hour == 14 || hour == 22 || hour == 2 ||  WorldMgr.current_blood <= 0)
                {
                    if (boss)
                    {
                        minute1 = System.DateTime.Now.AddMinutes(5.0);
                        int currentPVE_ID = WorldMgr.currentPVE_ID;
                        this.SendSystemNotice(string.Format("Chefe do mundo {0} terminou. A sala fechará após {1} minutos.", WorldMgr.name[currentPVE_ID], 5));
                        boss = false;
                    }
            ...
            EntertamentModeMgr.CheckOpenOrClose();
            int hour2 = DateTime.Now.Hour;
            if (hour2 > this.awardTime && WorldMgr.CanSendLightriddleAward)
            {
                WorldMgr.SendLightriddleTopEightAward();
            }
            if (hour2 > this.awardTime && WorldMgr.CanSendLuckyStarAward)
            {

            }
            if (hour2 > 1 && hour2 < this.awardTime)
            {
        ...
        {
            get => this._aSSState;
            set => this._aSSState = value;
        }

        public bool DailyAwardState
        {
            get => this._dailyAwardState;
            set => this._dailyAwardState = value;
        }

        public static CenterServer Instance => CenterServer._instance;
    }
}
