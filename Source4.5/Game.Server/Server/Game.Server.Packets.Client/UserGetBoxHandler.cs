      ...
      else
      {
        ...
        if (flag)
        {
          if (templateByCondition1 != null)
          {
            ...
            int honor = 0;
            int exp = 0;
            ItemBoxMgr.CreateItemBox(Convert.ToInt32(templateByCondition1.TemplateID), itemInfos, ref gold, ref point, ref giftToken, ref exp, ref honor);
            if (gold > 0)
              player.AddGold(gold);
            if (point > 0)
