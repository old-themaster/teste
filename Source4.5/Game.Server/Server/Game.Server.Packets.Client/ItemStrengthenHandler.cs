...
namespace Game.Server.Packets.Client
{
    ...
    public class ItemStrengthenHandler : IPacketHandler
    {
        ...
        public int HandlePacket(GameClient client, GSPacketIn packet)
        {
            ...
            double num3 = 0.0;
            double num4 = 0.0;
            if(mainItem.Template.CategoryID == 7 && mainItem.StrengthenLevel == GameProperties.GunLevelLimit ||mainItem.Template.CategoryID == 5 && mainItem.StrengthenLevel == GameProperties.ClothLevelLimit || mainItem.Template.CategoryID == 1 && mainItem.StrengthenLevel == GameProperties.HatLevelLimit)
            {
                client.Player.SendMessage("O Fortalecimento para esse item não esta permitido!");
                return 0;
                //Compila os emuladores primeiro gay e coloca no vps ou aonde tu vai testar pelo amor de jesus cristo
            }
            if (mainItem != null && mainItem.Template.CanStrengthen && mainItem.Count == 1)
            {
                ...
                if (itemInfoList.Count >= 1)
                {
                    ...
                    if (Math.Floor((num5 + num6 + num4 + num3) * 100.0) > (double)ItemStrengthenHandler.random.Next(10000))
                    {
                        ...
                        /*if (mainItem.Template.CategoryID == 7 && client.Player.Extra.CheckNoviceActiveOpen(NoviceActiveType.STRENGTHEN_COLTHES_ACTIVE))
                          client.Player.Extra.UpdateEventCondition(2, mainItem.StrengthenLevel);
                        if (mainItem.Template.CategoryID == 7 && client.Player.Extra.CheckNoviceActiveOpen(NoviceActiveType.STRENGTHEN_HAT_ACTIVE))
                          client.Player.Extra.UpdateEventCondition(2, mainItem.StrengthenLevel);*/
                    }
                    else
                    {
