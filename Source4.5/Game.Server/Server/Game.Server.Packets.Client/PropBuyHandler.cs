  ...
  public class PropBuyHandler : IPacketHandler
  {
    public int HandlePacket(GameClient client, GSPacketIn packet)
    {
      ...
      if (((IEnumerable<int>) PropItemMgr.PropFightBag).ToList<int>().Contains(shopItemInfoById.TemplateID) && shopItemInfoById != null)
      {
        ...
        ShopMgr.SetItemType(shopItemInfoById, type, ref damageScore, ref petScore, ref iTemplateID, ref iCount, ref gold, ref money, ref offer, ref gifttoken, ref num, ref num, ref hardCurrency, ref LeagueMoney, ref num);
        if (itemTemplate.CategoryID == 10)
        {
          ...
          if (gold <= playerCharacter.Gold && (money <= (playerCharacter.Money < 0 ? 0 : playerCharacter.Money) && offer <= playerCharacter.Offer && (gifttoken <= playerCharacter.GiftToken && num <= playerCharacter.medal && num <= playerCharacter.Ascension)))
          {
            ...
            if (client.Player.FightBag.AddItem(fromTemplate, 0))
            {
              ...
              client.Player.RemoveAscension(num);
            }
          }
          else
