...
namespace Game.Server.Statics
{
    public class LogMgr
    {
        ...
        private static Dictionary<int, long> m_player = new Dictionary<int, long>();
          ...
          (object) DateTime.Now
            ...
            catch (Exception ex)
            {
                if (!LogMgr.log.IsErrorEnabled)
                    return;
                LogMgr.log.Error((object)("LogMgr Error：LogDropItem @ " + ex?.ToString()));
            }
        }
          ...
          string BossWar)
        {
          ...
          (object) BossWar
            ...
            catch (Exception ex)
            {
                if (!LogMgr.log.IsErrorEnabled)
                    return;
                LogMgr.log.Error((object)("LogMgr Error：Fight @ " + ex?.ToString()));
            }
        }

        public static int AddMoney(int id, int money)
        {
            lock (LogMgr.m_player)
            {
                if (LogMgr.m_player.ContainsKey(id) && money > 0)
                {
                    LogMgr.m_player[id] += (long)money;
                    return money;
                }
            }
            return 0;
          ...
          int result)
        {
          ...
          (object) result
            ...
            catch (Exception ex)
            {
                if (!LogMgr.log.IsErrorEnabled)
                    return;
                LogMgr.log.Error((object)("LogMgr Error：ItemAdd @ " + ex?.ToString()));
            }
        }
          ...
          string goodsType)
        {
          ...
          (object) goodsType
            ...
            catch (Exception ex)
            {
                if (!LogMgr.log.IsErrorEnabled)
                    return;
                LogMgr.log.Error((object)("LogMgr Error：LogMoney @ " + ex?.ToString()));
            }
        }
        ...
        public static bool Setup(int gametype, int serverid, int areaid)
        {
            ...
            LogMgr.m_LogMoney.Columns.Add("Ascension", typeof(int));
