...
namespace Game.Server.Managers
{
    public class ConsortiaMgr
  {
    ...
    public static int CanConsortiaFight(int consortiaID1, int consortiaID2)
    {
      ...
      return consortiaInfo1 == null || consortiaInfo2 == null || (consortiaInfo1.Level < 3 || consortiaInfo2.Level < 3) ? -1 : ConsortiaMgr.FindConsortiaAlly(consortiaID1, consortiaID2);
    }
    ...
    public static bool ConsortiaShopUpGrade(int consortiaID, int shopLevel)
    {
      ...
      try
      {
        if (ConsortiaMgr._consortia.ContainsKey(consortiaID))
        {
          if (ConsortiaMgr._consortia[consortiaID].IsExist)
    ...
    public static int GetOffer(int cosortiaID1, int consortiaID2, eGameType gameType) => ConsortiaMgr.GetOffer(ConsortiaMgr.FindConsortiaAlly(cosortiaID1, consortiaID2), gameType);
    ...
    public static bool Init()
    {
      try
      {
        ...
        return ConsortiaMgr.LoadConsortia(ConsortiaMgr._consortia, ConsortiaMgr._consortiaBossConfigInfos);
      }
      catch (Exception ex)
      {
    ...
    private static bool Load(Dictionary<string, int> ally)
    {
      using (ConsortiaBussiness consortiaBussiness = new ConsortiaBussiness())
      {
        foreach (ConsortiaAllyInfo consortiaAllyInfo in consortiaBussiness.GetConsortiaAllyAll())
        {
          ...
          {
            string key = consortiaAllyInfo.Consortia1ID >= consortiaAllyInfo.Consortia2ID ? consortiaAllyInfo.Consortia2ID.ToString() + "&" + (object) consortiaAllyInfo.Consortia1ID : consortiaAllyInfo.Consortia1ID.ToString() + "&" + (object) consortiaAllyInfo.Consortia2ID;
    ...
    public static bool ReLoad()
    {
      try
      {
        ...
        if (ConsortiaMgr.Load(ally))
        {
          if (ConsortiaMgr.LoadConsortia(consortia, consortiaBossConfig))
          {
            ConsortiaMgr.m_lock.AcquireWriterLock(-1);
            try
            {
