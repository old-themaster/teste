    ...
    public class BaseRoom
    {
        ...
        public bool CanStart()
        {
            if (this.RoomType == eRoomType.Freedom)
            {
                ...
                return (uint)num1 > 0U && (uint)num2 > 0U;
            }
            int num = 0;
            for (int index = 0; index < 8; ++index)
            {
        ...
        public void UpdateRoomGameType()
        {
            switch (this.RoomType)
            {
                ...
                case eRoomType.EntertainmentRoom:
                    this.GameType = eGameType.EntertainmentRoom;
                    break;

                case eRoomType.EntertainmentRoomPK:
                    this.GameType = eGameType.EntertainmentRoomPK;
                    break;
            }
        }
        ...
        public void leaguetorneo()
        {
            if (this.gamePlayer_1 == null)
                return;
            GSPacketIn packet = new GSPacketIn((short)94);
            packet.WriteByte((byte)35);
            packet.WriteString("O torneio 2vs2 foi aberto chamando amigos dos pro players para criar a sala entre si. ");
            this.gamePlayer_1.Out.SendTCP(packet);
        ...
        public void SendNoTicketWarriorArena()
        {
            ...
            GSPacketIn packet = new GSPacketIn((short)94);
            packet.WriteByte((byte)35);
            packet.WriteString("Arena não ira começa devido alguém sem ingressos");
            this.gamePlayer_1.Out.SendTCP(packet);
        }
        ...
        public int GetDungeonTicketId(int mapId, eHardLevel level)
        {
            ...
            switch (mapId)
            {
                case 13: //Arena do Guerreiro
                    if (level == eHardLevel.Easy)
                        tickedID = 200619;
                    if (level == eHardLevel.Normal)
                        tickedID = 200620;
                    if (level == eHardLevel.Hard)
                        tickedID = 200621;
                    if (level == eHardLevel.Terror)
                        tickedID = 200622;
                    if (level == eHardLevel.Epic)
                        tickedID = 201105;
                    break;
                case 14: //Loucura da Copa 
                    if (level == eHardLevel.Normal)
                        tickedID = 201279;
                    if (level == eHardLevel.Hard)
                        tickedID = 201280;
        ...
        public eLevelLimits GetLevelLimit(GamePlayer player)
        {
            ...
            return player.PlayerCharacter.Grade <= 20 ? eLevelLimits.ElevenToTwenty : eLevelLimits.TwentyOneToThirty;
        }
        ...
        {
            string str = "Chưa lựa chọn?";
            string str2 = "<Mời tham gia Phó bản>";
            MapInfo mapInfo = MapMgr.FindMapInfo(this.MapId);
            if (mapInfo != null)
            {
                str = mapInfo.Name;
            }
            PveInfo pveInfoById = PveInfoMgr.GetPveInfoById(this.MapId);
            if (pveInfoById != null)
            {
                str = pveInfoById.Name + this.GetNameHardLv();
            }
            else
            {
                str2 = "<Mời tham gia đấu đội>";
            }
            return str2 + str;
        }
