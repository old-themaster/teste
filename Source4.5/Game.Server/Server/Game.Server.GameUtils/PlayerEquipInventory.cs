...
namespace Game.Server.GameUtils
{
    public class PlayerEquipInventory : PlayerInventory
    {
      ...
      20
        ...
        public void ClearLatentEnergy()
        {
            for (int i = 0; i < base.Capalility; i++)
            {
                if (m_items[i] != null && m_items[i].CanLatentEnergy() && m_items[i].latentEnergyCurStr != "0,0,0,0")
                {
                    if (DateTime.Now > m_items[i].latentEnergyEndTime)
                    {
                        m_items[i].latentEnergyCurStr = "0,0,0,0";
                        m_items[i].latentEnergyNewStr = "0,0,0,0";
                ...
                lock (this.m_lock)
                {
                    ...
                    for (int index = 0; index < 31; ++index)
                    {
                        ...
                        if (itemInfo != null)
                        {
                            ...
                            if (itemInfo.CanLatentEnergy())
                            {
                                if (itemInfo.latentEnergyEndTime > DateTime.Now)
                                {
                                    attack1 += Convert.ToInt32(itemInfo.latentEnergyCurStr.Split(',')[0]);
                                    defence1 += Convert.ToInt32(itemInfo.latentEnergyCurStr.Split(',')[1]);
                                    agility1 += Convert.ToInt32(itemInfo.latentEnergyCurStr.Split(',')[2]);
                                    lucky1 += Convert.ToInt32(itemInfo.latentEnergyCurStr.Split(',')[3]);
                                }
                            }
