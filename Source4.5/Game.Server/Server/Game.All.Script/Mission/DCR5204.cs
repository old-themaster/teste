...
namespace GameServerScript.AI.Messions
{
    public class DCR5204 : AMissionControl
    {
        ...
        private int bossId = 5231;

        private int npcId = 5232;

        private int npcHelperId1 = 5233;

        private int npcHelperId2 = 5234;
        ...
        public override void OnStartGame()
        {
            ...
            m_boss = Game.CreateBoss(bossId, 1700, 480, -1, 1, 0, "", config);
        ...
        private void CreateHelperNpc()
        {
            ...
            m_npcHelper = Game.CreateBoss(npcHelperId1, 190, 250, 1, 2, 0, "", config);
            m_npcHelper.Delay += 1;
            Game.SendHideBlood(m_npcHelper, 0);
            m_npcHelper.Say(LanguageMgr.GetTranslation("GameServerScript.AI.NPC.FiveHardElderFour.msg1"), 0, 3000, 2000);
        }
        ...
        private void MoveAllPlayer()
        {
            ...
            {
                p.SpeedMultX(18);
                p.StartSpeedMult(680 + Game.Random.Next(0, 50), p.Y, 0);
                //Console.WriteLine(string.Format("nick: {0} x: {1} y: {2)", p.PlayerDetail.PlayerCharacter.NickName, p.X, p.Y));
            }
        }
