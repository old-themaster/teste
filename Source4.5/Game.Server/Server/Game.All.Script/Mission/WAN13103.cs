...
namespace GameServerScript.AI.Messions
{
    ...
    {
        private int bossId = 13107; // minortaur

        private int npcHelpId = 13106;

        private int npcId = 13110; // hop hinh cu
        ...
        {
            if (m_tempBoss.X >= Game.Map.Bound.Width / 2)
            {
                // dung phia ben phai => nhin sang ben trai
            ...
            Game.SendObjectFocus(m_tempBoss, 1, 1000, 0);
            m_tempBoss.Say("Con tró toàn phá rối ta.", 0, 1000);
        ...
        public override void OnDied()
        {
            ...
            if (boss.IsLiving == false)
            {
                ...
                m_tempBoss = Game.CreateBoss(bossId, boss.X, boss.Y, boss.Direction, 1, "", config);
                Game.RemoveLiving(boss.Id);
                Game.RemoveLiving(bossHelp.Id);

                Game.SendHideBlood(m_tempBoss, 0);
                
                m_tempBoss.CallFuction(CreateEffectEndGame, Game.GetWaitTimerLeft());
            }
        }
        */
    }
