...
{
    public class CPN7101 : AMissionControl
    {
        private List<SimpleNpc> redNpc = new List<SimpleNpc>();

        private List<SimpleNpc> blueNpc = new List<SimpleNpc>();

        private List<Point> m_pointRed = new List<Point>() { new Point(958, 950), new Point(1400, 950), new Point(1034, 950), new Point(1472, 950), new Point(1034, 950), new Point(1472, 950) };

        private List<Point> m_pointBlue = new List<Point>() { new Point(1150, 950), new Point(1346, 950) };

        private PhysicalObj m_kingMoive;

        private int m_totalRed = 15;

        private int m_totalBlue = 5;

        private int maxRedOnMap = 6;

        private int maxBlueOnMap = 2;

        private int livingRed = 0;

        private int livingBlue = 0;

        private int redNpcID = 7102;

        private int blueNpcID = 7101;
        ...
        private void RespawnRedNpc(int count)
        {
            for (int i = 0; i < count; i++)
            {
                ...
                redNpc.Add(Game.CreateNpc(redNpcID, point.X, point.Y, 1));
            }
        }
        ...
        private void RespawnBlueNpc(int count)
        {
            for (int i = 0; i < count; i++)
            {
                ...
                blueNpc.Add(Game.CreateNpc(redNpcID, point.X, point.Y, 1));
            }
        }
