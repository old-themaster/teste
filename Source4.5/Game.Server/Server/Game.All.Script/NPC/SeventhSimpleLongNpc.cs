...
{
    public class SeventhSimpleLongNpc : ABrain
	{
		private int m_attackTurn = 0;

		private PhysicalObj effect = null;
		...
		public override void OnStartAttacking()
		{
			...
			{
				if (current.IsLiving && current.X < 800)
				{
		...
		private void GoMovie()
		{
			...
			{
				this.effect = ((PVEGame)base.Game).Createlayer(current.X, current.Y, "moive", "asset.game.seven.cao", "out", 1, 0);
				this.effect.PlayMovie("in", 1000, 0);
			}
		}
	}
}
