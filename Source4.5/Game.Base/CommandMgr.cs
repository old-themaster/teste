	...
	public class CommandMgr
	{
		...
		public static bool LoadCommands()
		{
			...
			foreach (Assembly item in new ArrayList(ScriptMgr.Scripts))
			{
				...
				{
					log.Debug("ScriptMgr: Searching for commands in " + item.GetName());
				}
						...
						try
						{
							...
							for (int j = 0; j < customAttributes.Length; j++)
							{
								...
								foreach (string b in disabledarray)
								{
									...
									{
										flag = true;
										log.Info("Will not load command " + cmdAttribute.Cmd + " as it is disabled in game properties");
										break;
									}
								}
								if (!flag)
								{
									if (m_cmds.ContainsKey(cmdAttribute.Cmd))
									{
										log.Info(cmdAttribute.Cmd + " from " + item.GetName() + " has been suppressed, a command of that type already exists!");
									}
									else
									{
										if (log.IsDebugEnabled)
										{
											log.Debug("Load: " + cmdAttribute.Cmd + "," + cmdAttribute.Description);
										}
