                    ...
                    else
                    {
                        if (!proxyRoom.IsCrossZone)
                        {
                            if (proxyRoom.PickUpCount >= 4 && !proxyRoom.startWithNpc && (!proxyRoom.isAutoBot && proxyRoom.RoomType == eRoomType.Match) && proxyRoom.PlayerCount == 1)
                            {
                                ...
                                proxyRoom.Client.SendBeginFightNpc(proxyRoom.selfId, 0, (int)proxyRoom.GameType, proxyRoom.NpcId);
            ...
            foreach (ProxyRoom proxyRoom_0 in waitMatchRoomUnsafe)
            {
                ...
                else if (!proxyRoom_0.IsCrossZone)
                {
                    if (proxyRoom_0.PickUpCount == num1 && !proxyRoom_0.startWithNpc && (!proxyRoom_0.isAutoBot && proxyRoom_0.RoomType == eRoomType.Match))
                    {
                        ...
                        proxyRoom_0.Client.SendBeginFightNpc(proxyRoom_0.selfId, 0, (int)proxyRoom_0.GameType, proxyRoom_0.NpcId);
