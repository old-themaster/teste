...
// Assembly location: C:\WONDERTANK vReZero\Emulator\Fight\Fighting.Server.dll
...
namespace Fighting.Server
{
    public class FightServer : BaseServer
    {
        ...
        private FightServer(FightServerConfig config) => this.m_config = config;
        ...
        public override bool Start()
        {
            ...
            try
            {
                ...
                if (!this.InitComponent(this.InitSocket(IPAddress.Parse(this.m_config.Ip), this.m_config.Port), "InitSocket Port:" + (object)this.m_config.Port) || !this.InitComponent(this.RecompileScripts(), "Recompile Scripts") || (!this.InitComponent(this.StartScriptComponents(), "Script components") || !this.InitComponent(ProxyRoomMgr.Setup(), "RoomMgr.Setup")) || (!this.InitComponent(GameMgr.Setup(0, 4), "GameMgr.Setup") || !this.InitComponent(MapMgr.Init(), "MapMgr Init") || (!this.InitComponent(ItemMgr.Init(), "ItemMgr Init") || !this.InitComponent(PropItemMgr.Init(), "PropItemMgr Init"))) || (!this.InitComponent(BallMgr.Init(), "BallMgr Init") || !this.InitComponent(BallConfigMgr.Init(), "BallConfigMgr Init") || (!this.InitComponent(DropMgr.Init(), "DropMgr Init") || !this.InitComponent(NPCInfoMgr.Init(), "NPCInfoMgr Init")) || (!this.InitComponent(WindMgr.Init(), "WindMgr Init") || !this.InitComponent(LanguageMgr.Setup(""), "LanguageMgr Init"))))
                    return false;
        ...
        public FightServerConfig Configuration => this.m_config;

        public static FightServer Instance => FightServer.m_instance;
    }
}
