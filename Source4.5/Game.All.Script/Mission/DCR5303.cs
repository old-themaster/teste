...
namespace GameServerScript.AI.Messions
{
    public class DCR5303 : AMissionControl
    {
        ...
        private int npcId1 = 5322;

        private int npcId2 = 5323;

        private int npcId3 = 5324;

        private int npcId4 = 5304;
        ...
        public override void OnStartGame()
        {
            ...
            m_boss = Game.CreateBoss(bossId, 1000, 500, 1, 1, 0, "", config);
            m_boss.SetRelateDemagemRect(m_boss.NpcInfo.X, m_boss.NpcInfo.Y, m_boss.NpcInfo.Width, m_boss.NpcInfo.Height);

            m_boss.Say(LanguageMgr.GetTranslation("GameServerScript.AI.NPC.FiveTerrorBossThird.msg1"), 0, 1000);
        ...
        private void CreateEffectEndGame()
        {
            ...
            m_tempBoss.Say(LanguageMgr.GetTranslation("GameServerScript.AI.NPC.FiveTerrorBossThird.msg19"), 0, 1000);
        ...
        {
            //m_tempBoss.Die();
            SimpleNpc npc = ((PVEGame)Game).CreateNpc(npcId4, 179, 552, 1, 1, "standC", ((PVEGame)Game).BaseLivingConfig());
            npc.PlayMovie("cool", 1000, 0);
            npc.Say(LanguageMgr.GetTranslation("GameServerScript.AI.NPC.FiveTerrorElderThird.msg1"), 0, 4000, 2000);
        ...
        public override void OnDied()
        {
            ...
            if (m_boss.IsLiving == false)
            {
                ...
                m_tempBoss = Game.CreateBoss(bossId, m_boss.X, m_boss.Y, m_boss.Direction, 1, 0, "", config);
                Game.RemoveLiving(m_boss.Id);

                Game.SendHideBlood(m_tempBoss, 0);
                m_tempBoss.MoveTo(1000, 485, "fly", Game.GetWaitTimerLeft(), CreateEffectEndGame, 10);
            }
        }
    }
}
