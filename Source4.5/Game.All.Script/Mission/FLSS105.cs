...
namespace GameServerScript.AI.Messions
{
    public class FLSS105 : AMissionControl
    {
        ...
        private int needHitCount = 3;

        private int currTurnID = 0;

        private int redNpcID = 6;
        ...
        public void CreateNpc()
        {
            ...
            if (createNpcCount <= maxTurnSize)
            {
                if (isCreateNpcFirst)
                {
                    someNpc = Game.CreateNpc(redNpcID, 932, 769, 0, 1, config);
                ...
                someNpc = Game.CreateNpc(redNpcID, createNpcX[x].X, createNpcX[x].Y, 0, 1, config);
                someNpc.SetRelateDemagemRect(-25, -55, 45, 55);
                createNpcCount++;
                currTurnID++;
                Game.WaitTime(0);
