...
namespace Center.Server
{
  public class ServerClient : BaseClient
  {
    ...
    public void HandkeItemStrengthen(GSPacketIn pkg) => this._svr.SendToALL(pkg, this);

    public void HandleBigBugle(GSPacketIn pkg) => this._svr.SendToALL(pkg, this);
    ...
    public void HandleConsortiaFight(GSPacketIn pkg) => this._svr.SendToALL(pkg);
    ...
    public void HandleConsortiaUpGrade(GSPacketIn pkg) => this._svr.SendToALL(pkg, this);

    public void HandleChatConsortia(GSPacketIn pkg) => this._svr.SendToALL(pkg, this);

    public void HandleChatPersonal(GSPacketIn pkg) => this._svr.SendToALL(pkg);
    ...
    public void HandleFirendResponse(GSPacketIn pkg) => this._svr.SendToALL(pkg, this);

    public void HandleFriend(GSPacketIn pkg) => this._svr.SendToALL(pkg, this);

    public void HandleFriendState(GSPacketIn pkg) => this._svr.SendToALL(pkg, this);
    ...
    public void HandleRecvConsortiaBossUpdateBlood(GSPacketIn pkg) => ConsortiaBossMgr.UpdateBlood(pkg.ReadInt(), pkg.ReadInt());

    public void HandleRecvConsortiaBossUpdateRank(GSPacketIn pkg) => ConsortiaBossMgr.UpdateRank(pkg.ReadInt(), pkg.ReadInt(), pkg.ReadInt(), pkg.ReadInt(), pkg.ReadString(), pkg.ReadInt());
    ...
    private void HandleUserPrivateMsg(GSPacketIn pkg, int playerid) => LoginMgr.GetServerClient(playerid)?.SendTCP(pkg);

    public void HandleUserPublicMsg(GSPacketIn pkg) => this._svr.SendToALL(pkg, this);
    ...
    public void HandleWorldBossPrivateInfo(GSPacketIn pkg) => this._svr.SendPrivateInfo(pkg.ReadString());
    ...
    public void HandleWorldBossUpdateBlood(GSPacketIn pkg)
    {
      ...
      this._svr.SendUpdateWorldBlood();
        ...
        public void HandleEnterModeGetPoint(GSPacketIn pkg)
        {
            ...
            GSPacketIn @in = EntertamentModeMgr.SendGetPoint(player);
            this.SendTCP(@in);
        }

        public void HandleEnterModeStartOrStop(GSPacketIn pkg)
        {
            GSPacketIn @in = EntertamentModeMgr.SendOpenOrClose(false);
            this.SendTCP(@in);
        }
        ...
        public void HandleEnterModeUpdatePoint(GSPacketIn pkg)
        {
            PlayerInfo player = new PlayerInfo
            {
                ID = pkg.ReadInt(),
                NickName = pkg.ReadString(),
                EntertamentPoint = pkg.ReadInt()
            };
            EntertamentModeMgr.UpdatePoint(player, player.EntertamentPoint);
        }
      ...
      else
      {
        switch (code)
        {
          ...
          case 190:
            ...
            break;
          case 0xc0:
            this.HandleEnterModeStartOrStop(pkg);
            break;
          default:
            if (code == (short) 240)
    ...
    public void SendKitoffUser(int playerid) => this.SendKitoffUser(playerid, LanguageMgr.GetTranslation("Center.Server.SendKitoffUser"));
