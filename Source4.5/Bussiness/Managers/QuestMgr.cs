...
namespace Bussiness.Managers
{
  public class QuestMgr
  {
    ...
    public static List<AchievementConditionInfo> GetAchievementCondiction(
      AchievementInfo info) => QuestMgr.dictionary_4.ContainsKey(info.ID) ? QuestMgr.dictionary_4[info.ID] : (List<AchievementConditionInfo>) null;

    public static List<AchievementGoodsInfo> GetAchievementGoods(
      AchievementInfo info) => QuestMgr.dictionary_5.ContainsKey(info.ID) ? QuestMgr.dictionary_5[info.ID] : (List<AchievementGoodsInfo>) null;

    public static List<AchievementInfo> GetAllAchievements() => QuestMgr.dictionary_3.Values.ToList<AchievementInfo>();
    ...
    public static List<QuestConditionInfo> GetQuestCondiction(QuestInfo info) => QuestMgr.m_questcondiction.ContainsKey(info.ID) ? QuestMgr.m_questcondiction[info.ID] : (List<QuestConditionInfo>) null;

    public static List<QuestAwardInfo> GetQuestGoods(QuestInfo info) => QuestMgr.m_questgoods.ContainsKey(info.ID) ? QuestMgr.m_questgoods[info.ID] : (List<QuestAwardInfo>) null;

    public static AchievementInfo GetSingleAchievement(int id) => QuestMgr.dictionary_3.ContainsKey(id) ? QuestMgr.dictionary_3[id] : (AchievementInfo) null;

    public static QuestInfo GetSingleQuest(int id) => QuestMgr.m_questinfo.ContainsKey(id) ? QuestMgr.m_questinfo[id] : (QuestInfo) null;
