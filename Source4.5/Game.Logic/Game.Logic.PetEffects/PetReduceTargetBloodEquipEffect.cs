...
// Type: Game.Logic.PetEffects.PetReduceTargetBloodEquipEffect
...
namespace Game.Logic.PetEffects
{
  public class PetReduceTargetBloodEquipEffect : BasePetEffect
  {
    ...
    public PetReduceTargetBloodEquipEffect(
      int count,
      int probability,
      int type,
      int skillId,
      int delay,
      string elementID)
      : base(ePetEffectType.PetReduceTargetBloodEquipEffect, elementID)
    {
      this.m_count = count;
      ...
      int criticalAmount)
    {
      if (!this.IsTrigger || living == target || !(target is Player))
        return;
      target.AddPetEffect((AbstractPetEffect) new PetReduceBloodEquip(this.m_count, this.m_currentId, this.Info.ID.ToString()), 0);
      this.IsTrigger = false;
    }
