  ...
  public class PetAttackedRecoverBloodEquipEffect : BasePetEffect
  {
    ...
    private void ChangeProperty(Player player)
    {
      ...
      foreach (Living allTeamPlayer in player.Game.GetAllTeamPlayers((Living) player))
        allTeamPlayer.AddPetEffect((AbstractPetEffect) new PetAttackedRecoverBloodEquip(this.m_count, this.m_currentId, this.Info.ID.ToString()), 0);
    }
    ...
    {
      PetAttackedRecoverBloodEquipEffect ofType = living.PetEffectList.GetOfType(ePetEffectType.PetAttackedRecoverBloodEquipEffect) as PetAttackedRecoverBloodEquipEffect;
      if (ofType == null)
