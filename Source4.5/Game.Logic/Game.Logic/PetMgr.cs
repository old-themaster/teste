...
namespace Game.Logic
{
	public class PetMgr
	{
		...
		public static PetStarExpInfo GetPetStarExp(int oldId)
		{
			...
			PetMgr.readerWriterLock_0.AcquireReaderLock(10000);
			try
			{
			...
			{
				PetMgr.readerWriterLock_0.ReleaseReaderLock();
			}
			return null;
		}
	  ...
	  int playerLevel)
		...
		public static int UpdateEvolution(int TemplateID, int lv)
		{
			...
			return petTemplate != null ? petTemplate.TemplateID : TemplateID;
		}
			...
			List<int> intList = new List<int>()
	  {
		...
		{
			int AdoptCount = Convert.ToInt32(PetMgr.FindConfig("AdoptCount").Value);
			List<UsersPetInfo> infos = new List<UsersPetInfo>();
			List<PetTemplateInfo> petIdInfos = null;
			int index = 0;
			while (index < AdoptCount)
			{
				if (DropInventory.GetPetDrop(613, 1, ref petIdInfos) && petIdInfos != null)
				{
					int id = PetMgr.rand.Next(petIdInfos.Count);
					UsersPetInfo info = PetMgr.CreatePet(petIdInfos[id], userID, index, playerLevel);
					info.IsExit = true;
					infos.Add(info);
					index++;
				}
		...
		public static UsersPetInfo CreateNewPet(int vipLv)
		{
			...
			usersPetinfo.ID = 0;
			usersPetinfo.BloodGrow = PetMgr.random_0.Next(minValue2, maxValue2);
			usersPetinfo.AttackGrow = PetMgr.random_0.Next(minValue1, maxValue1);
			usersPetinfo.DefenceGrow = PetMgr.random_0.Next(minValue1, maxValue1);
			usersPetinfo.AgilityGrow = PetMgr.random_0.Next(minValue1, maxValue1);
			usersPetinfo.LuckGrow = PetMgr.random_0.Next(minValue1, maxValue1);
			usersPetinfo.DamageGrow = 0;
			usersPetinfo.GuardGrow = 0;
			double num1 = (double)PetMgr.random_0.Next(54, 61) * 0.1;
			double num2 = (double)PetMgr.random_0.Next(9, 13) * 0.1;
			usersPetinfo.Blood = (int)((double)(PetMgr.random_0.Next(minValue2, maxValue2) / 10) * 0.1 * num1);
			usersPetinfo.Attack = (int)((double)(PetMgr.random_0.Next(minValue1, maxValue1) / 10) * 0.1 * num2);
			usersPetinfo.Defence = (int)((double)(PetMgr.random_0.Next(minValue1, maxValue1) / 10) * 0.1 * num2);
			usersPetinfo.Agility = (int)((double)(PetMgr.random_0.Next(minValue1, maxValue1) / 10) * 0.1 * num2);
			usersPetinfo.Luck = (int)((double)(PetMgr.random_0.Next(minValue1, maxValue1) / 10) * 0.1 * num2);
			usersPetinfo.Damage = 0;
			usersPetinfo.Guard = 0;
			usersPetinfo.Hunger = 10000;
			usersPetinfo.TemplateID = petTemplate.TemplateID;
			usersPetinfo.Name = petTemplate.Name;
			usersPetinfo.UserID = -1;
			usersPetinfo.Place = -1;
			usersPetinfo.Level = 60;
			usersPetinfo.Skill = PetMgr.UpdateSkillPet(60, petTemplate.TemplateID, 60);
		...
		public static UsersPetInfo CreateNewPet()
		{
			...
			usersPetinfo.Skill = PetMgr.UpdateSkillPet(60, petTemplate.TemplateID, 60);
			usersPetinfo.SkillEquip = PetMgr.ActiveEquipSkill(1);
			PetMgr.CreateNewPropPet(ref usersPetinfo);
			return usersPetinfo;
		}

		public static void CreateNewPropPet(ref UsersPetInfo pet)
		{
			PetConfig rateConfig = PetMgr.FindConfig("PropertiesRate");
			int rate_prop = (rateConfig == null) ? 100 : int.Parse(rateConfig.Value);
			double[] propInfo = new double[5];
			PetTemplateInfo info = PetMgr.FindPetTemplate(pet.TemplateID);
			if (info == null)
			{
				return;
			}
