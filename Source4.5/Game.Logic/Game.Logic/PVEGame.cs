...
namespace Game.Logic
{
    public class PVEGame : BaseGame
    {
        ...
        public SimpleBoss CreateBoss(int npcId, int x, int y, int direction, int type, int bloodInver, string action, LivingConfig config)
        {
            NpcInfo npcInfoById = NPCInfoMgr.GetNpcInfoById(npcId);
            ...
            {
                simpleBoss.Reset();            
                if (simpleBoss.Config.IsWorldBoss && this.WorldbossBood < 2147483647)
                {
        ...
        public void GameOver()
        {
            ...
            foreach (Player player in allFightPlayers)
            {
                ...
                player.Ready = false;
                // player.Reset(); // this ?? bug de morre e passa a vez
        ...
        public void GameOverAllSession()
        {
            ...
            {
                // item.PlayerDetail.OnGameOver(this, IsWin, item.GainGP, isSpanArea: false, CoupleFight(item), item.Blood, base.PlayerCount);
                item.PlayerDetail.OnGameOver(this, this.IsWin, item.GainGP);
            }
            int canTakeOut = 1;
            if (!IsWin)
            {
            ...
            {
                item2.CanTakeOut = canTakeOut;
                gSPacketIn.WriteInt(item2.PlayerDetail.PlayerCharacter.ID);
                gSPacketIn.WriteInt(item2.TotalAllKill);
                gSPacketIn.WriteInt(item2.TotalAllHurt);
                gSPacketIn.WriteInt(item2.TotalAllScore);
                gSPacketIn.WriteInt(item2.TotalAllCure);
                ...
                gSPacketIn.WriteInt(0);
                gSPacketIn.WriteInt(0);
                gSPacketIn.WriteInt(item2.TotalAllExperience);
        ...
        public void TakeSnow()
        {
            ...
            if (itemTemplate == null)
                return;
            SqlDataProvider.Data.ItemInfo fromTemplate = SqlDataProvider.Data.ItemInfo.CreateFromTemplate(itemTemplate, 1, 101);
            fromTemplate.IsBinds = true;
            int count = this.Random.Next(3, 9);
            string msg = "";
            foreach (Player player in this.GetAllFightPlayers())
            {
                player.PlayerDetail.AddTemplate(fromTemplate, fromTemplate.Template.BagType, count, eGameView.dungeonTypeGet);
                msg = msg + string.Format("Tu recebes {0} x{1}, neste ataque.", (object)fromTemplate.Template.Name, (object)count);
