...
// Type: Tank.Request.LoadMapsItems
  ...
  public class LoadMapsItems : IHttpHandler
  {
    ...
    public static string Bulid(HttpContext context)
    {
      ...
      try
      {
        ...
        {
          foreach (MapInfo all in mapBussiness.GetAllMap())
            result.Add((object) FlashUtils.CreateMapInfo(all));
        }
        flag = true;
        str = "Success!";
      }
      ...
      {
        LoadMapsItems.log.Error((object) nameof (LoadMapsItems), ex);
      }
      result.Add((object) new XAttribute((XName) "value", (object) flag));
      result.Add((object) new XAttribute((XName) "message", (object) str));
      return csFunction.CreateCompressXml(context, result, nameof (LoadMapsItems), true);
    }
